"""Comprehensive tests for the asynchronous AsyncAvatronClient."""
...
from avatron.exceptions import (
    AuthenticationError,
    AuthorizationError,
    ConflictError,
    NotFoundError,
...
class TestErrorHandling:
    ...
    async def test_server_error_500(self, async_client, mock_api):
        ...
        with pytest.raises(AvatronError) as exc_info:
            await async_client.list_gates()
...
class TestContextManager:
    async def test_async_context_manager(self, mock_api):
        ...
        async with AsyncAvatronClient(api_key=API_KEY, base_url=BASE_URL) as client:
